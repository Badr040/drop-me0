        ...
        <div className="max-w-7xl mx-auto px-4 py-12">
          ...
          <div className="bg-card rounded-2xl shadow-sm p-6 max-w-md mx-auto mb-8">
            <div className="flex items-center justify-between mb-2">
              ...
              <button className="text-primary text-sm hover:underline">History</button>
            </div>
            <div className="text-center">
              <span className="text-3xl font-bold text-primary">600 Points</span>
              <p className="text-sm text-muted mt-1">Equivalent Value: 60 EGP</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            ...
            {showBankForm && (
              <div className="bg-card rounded-2xl shadow-sm p-6">
                <h3 className="font-semibold text-center mb-4">Bank Transfer</h3>
                <form className="space-y-4">
                  <div>
                    <label className="block text-sm text-muted mb-1">Name</label>
                    <input type="text" className="w-full px-3 py-2 border border-border rounded-lg" />
                  </div>
                  <div>
            ...
            {showWalletForm && (
              <div className="bg-card rounded-2xl shadow-sm p-6">
                <h3 className="font-semibold text-center mb-4">Wallet Transfer</h3>
                <form className="space-y-4">
                  <div>
                    <label className="block text-sm text-muted mb-1">Name</label>
                    <input type="text" className="w-full px-3 py-2 border border-border rounded-lg" />
                  ...
                  <div>
                    <label className="block text-sm text-muted mb-1">Wallet Number</label>
                    <input type="text" className="w-full px-3 py-2 border border-border rounded-lg" />
                  </div>
                  <button
                    type="button"
            ...
            {showSuccess && (
              <div className="bg-card rounded-2xl shadow-sm p-6 text-center">
                <h3 className="font-semibold mb-4">
                  {selectedWallet === "bank" ? "Bank Transfer" : "Wallet Transfer"}
                </h3>
                <div className="w-16 h-16 rounded-full bg-secondary/20 flex items-center justify-center mx-auto mb-4">
                  <Check className="w-8 h-8 text-secondary" />
                </div>
                <p className="text-secondary font-semibold mb-2">Conversion Successful</p>
                <p className="text-sm text-muted">Your points have been converted successfully.</p>
              </div>
            )}
          </div>
        </div>
